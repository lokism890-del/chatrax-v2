...
const AVAILABLE_TAGS = [
  { id: 'vip', label: 'VIP', color: 'bg-red-500/20 text-red-400 border-red-500/30' },
  { id: 'refund', label: 'Refund', color: 'bg-yellow-500/20 text-yellow-400 border-yellow-500/30' },
  { id: 'wholesale', label: 'Wholesale', color: 'bg-purple-500/20 text-purple-400 border-purple-500/30' },
...
export default function Dashboard() {
  ...
  useEffect(() => { if (presenceChannelRef.current?.state === 'joined') presenceChannelRef.current.track({ agentName: settings.adminName, leadId: selectedLead?.id || null }); }, [selectedLead, settings.adminName]);
  useEffect(() => { messagesEndRef.current?.scrollIntoView({ behavior: 'smooth' }); }, [messages]);

  useEffect(() => {
    fetchLeads(); fetchStats(); fetchQuickReplies(); fetchAuditLogs();
    const channel = supabase.channel('realtime-customers').on('postgres_changes', { event: '*', schema: 'public', table: 'customers' }, () => fetchLeads()).subscribe();
    return () => { supabase.removeChannel(channel); };
  }, []);
    ...
    <div className={`flex h-screen w-screen text-zinc-100 font-sans relative overflow-hidden selection:bg-white/20 ${theme === 'grey' ? 'theme-grey' : theme === 'black' ? 'theme-black' : ''}`}>
      ...
      <div className="w-64 border-r border-white/10 bg-[#111827]/90 backdrop-blur-3xl flex flex-col z-40 shrink-0 h-full">
        ...
        <div className="flex-1 py-6 px-4 space-y-2 overflow-y-auto custom-scrollbar">
          ...
          <button onClick={() => setActiveView('conversations')} className={`w-full flex items-center justify-between px-4 py-3 rounded-xl transition-all duration-300 font-medium text-sm ${activeView === 'conversations' ? `${activeTheme.bgSubtle} ${activeTheme.text} ${activeTheme.border} border shadow-inner font-semibold` : 'text-zinc-300 hover:bg-white/5 hover:text-white'}`}>
             <div className="flex items-center gap-3"><MessageSquare className="w-4 h-4" /> Conversations</div>
             <span className={`${activeTheme.bgSubtle} ${activeTheme.text} border ${activeTheme.border} text-[10px] px-2 py-0.5 rounded-full font-bold`}>{activeConversationsCount}</span>
          </button>
          
          {userRole === 'ADMIN' && (
             <button onClick={() => setActiveView('campaigns')} className={`w-full flex items-center gap-3 px-4 py-3 rounded-xl transition-all duration-300 font-medium text-sm ${activeView === 'campaigns' ? `${activeTheme.bgSubtle} ${activeTheme.text} ${activeTheme.border} border shadow-inner font-semibold` : 'text-zinc-300 hover:bg-white/5 hover:text-white'}`}><Megaphone className="w-4 h-4" /> Campaigns</button>
          )}
          
          <button onClick={() => setActiveView('templates')} className={`w-full flex items-center gap-3 px-4 py-3 rounded-xl transition-all duration-300 font-medium text-sm ${activeView === 'templates' ? `${activeTheme.bgSubtle} ${activeTheme.text} ${activeTheme.border} border shadow-inner font-semibold` : 'text-zinc-300 hover:bg-white/5 hover:text-white'}`}><LayoutTemplate className="w-4 h-4" /> Templates</button>
          
          {userRole === 'ADMIN' && (
             <>
                <button onClick={() => setActiveView('analytics')} className={`w-full flex items-center gap-3 px-4 py-3 rounded-xl transition-all duration-300 font-medium text-sm ${activeView === 'analytics' ? `${activeTheme.bgSubtle} ${activeTheme.text} ${activeTheme.border} border shadow-inner font-semibold` : 'text-zinc-300 hover:bg-white/5 hover:text-white'}`}><BarChart2 className="w-4 h-4" /> Analytics</button>
                <button onClick={() => setActiveView('settings')} className={`w-full flex items-center gap-3 px-4 py-3 rounded-xl transition-all duration-300 font-medium text-sm ${activeView === 'settings' ? `${activeTheme.bgSubtle} ${activeTheme.text} ${activeTheme.border} border shadow-inner font-semibold` : 'text-zinc-300 hover:bg-white/5 hover:text-white'}`}><Settings className="w-4 h-4" /> Settings</button>
             </>
          )}
        </div>
        ...
        <div className="p-4 border-t border-white/10 bg-[#0F172A]/40 flex flex-col gap-3 shrink-0">
          ...
          <button onClick={toggleRole} className={`w-full py-2 rounded-lg text-[10px] font-bold uppercase tracking-widest transition-all border ${userRole === 'ADMIN' ? `${activeTheme.bgSubtle} ${activeTheme.text} ${activeTheme.border} ${activeTheme.hoverBg}` : 'bg-white/5 text-zinc-400 border-white/10 hover:bg-white/10'}`}>
             Switch to {userRole === 'ADMIN' ? 'Agent' : 'Admin'} Mode
          </button>
          
          <div className="flex items-center justify-between p-2.5 rounded-xl bg-white/5 border border-white/5 mt-1">
             <div className="flex items-center gap-3 overflow-hidden">
                <div className="w-8 h-8 shrink-0 rounded-full bg-gradient-to-tr from-sky-500 to-blue-600 flex items-center justify-center font-bold text-white text-xs">
             ...
             <button onClick={handleLogOut} className="p-2 hover:bg-red-500/10 rounded-lg text-zinc-400 hover:text-red-400 transition-all"><LogOut className="w-3.5 h-3.5" /></button>
          </div>
        </div>
      </div>
          ...
          <div className="flex flex-col h-full animate-[fade-in_0.3s_ease-out]">
            <div className="h-20 flex items-center justify-between px-6 border-b border-white/10 bg-[#111827]/80 backdrop-blur-xl shrink-0">
              ...
              <div className="flex items-center gap-3">
                 <div className={`flex items-center gap-2 bg-[#1F2937]/80 border border-white/10 rounded-lg px-3 py-1.5 text-[10px] font-bold tracking-wider ${activeTheme.text} shadow-inner`}>
                    <div className={`w-1.5 h-1.5 rounded-full ${activeTheme.bg} animate-led border border-white/20`}></div>
                    LIVE SYNC • {currentTime || "CONNECTING..."}
                 </div>
                 <button onClick={() => setActiveView('campaigns')} className={`bg-gradient-to-r ${activeTheme.gradient} ${activeTheme.hoverGradient} text-white font-semibold text-xs px-4 py-1.5 rounded-lg ${activeTheme.shadow} transition-all flex items-center gap-1.5 hover:-translate-y-0.5`}>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-6 shrink-0">
                  <div className="bg-[#1F2937]/70 backdrop-blur-2xl border border-white/10 rounded-2xl p-5 shadow-xl relative overflow-hidden">
                     ...
                     <div className="flex justify-between items-start mb-3 pl-2">
                        <div className={`w-8 h-8 rounded-xl ${activeTheme.bgSubtle} border ${activeTheme.border} flex items-center justify-center ${activeTheme.text}`}><ShoppingBag className="w-4 h-4"/></div>
                        <span className={`text-[9px] font-bold ${activeTheme.text} ${activeTheme.bgSubtle} px-2 py-0.5 rounded-full flex items-center gap-1`}><TrendingUp className="w-2.5 h-2.5"/> {newOrdersPct}%</span>
                     </div>
                     <p className="text-[10px] font-semibold tracking-widest text-zinc-400 uppercase mb-0.5 pl-2">New Orders</p>
                     <h3 className="text-2xl font-bold text-white tracking-tight pl-2 mt-0.5">{newOrdersCount}</h3>
                  </div>

                  <div className="bg-[#1F2937]/70 backdrop-blur-2xl border border-white/10 rounded-2xl p-5 shadow-xl relative overflow-hidden">
                     <div className="absolute top-0 left-0 w-1 h-full bg-sky-500" />
                     <div className="flex justify-between items-start mb-3 pl-2">
                        <div className="w-8 h-8 rounded-xl bg-sky-500/10 border border-sky-500/20 flex items-center justify-center text-sky-400"><Activity className="w-4 h-4"/></div>
                        <span className="text-[9px] font-bold text-sky-400 bg-sky-500/10 px-2 py-0.5 rounded-full flex items-center gap-1"><TrendingUp className="w-2.5 h-2.5"/> {activePct}%</span>
                     </div>
                     <p className="text-[10px] font-semibold tracking-widest text-zinc-400 uppercase mb-0.5 pl-2">Active Contacts</p>
                     <h3 className="text-2xl font-bold text-white tracking-tight pl-2 mt-0.5">{activeCount}</h3>
                  </div>

                  <div className="bg-[#1F2937]/70 backdrop-blur-2xl border border-white/10 rounded-2xl p-5 shadow-xl relative overflow-hidden">
                     <div className="absolute top-0 left-0 w-1 h-full bg-lime-500" />
                     <div className="flex justify-between items-start mb-3 pl-2">
                        <div className="w-8 h-8 rounded-xl bg-lime-500/10 border border-lime-500/20 flex items-center justify-center text-lime-400"><ShieldCheck className="w-4 h-4"/></div>
                        <span className="text-[9px] font-bold text-lime-400 bg-lime-500/10 px-2 py-0.5 rounded-full flex items-center gap-1"><TrendingUp className="w-2.5 h-2.5"/> {resolvedPct}%</span>
                     </div>
                     <p className="text-[10px] font-semibold tracking-widest text-zinc-400 uppercase mb-0.5 pl-2">Resolved Leads</p>
                     <h3 className="text-2xl font-bold text-white tracking-tight pl-2 mt-0.5">{resolvedCount}</h3>
                  </div>

                  <div className="bg-[#1F2937]/70 backdrop-blur-2xl border border-white/10 rounded-2xl p-5 shadow-xl relative overflow-hidden">
                     <div className="absolute top-0 left-0 w-1 h-full bg-amber-500" />
                     <div className="flex justify-between items-start mb-3 pl-2">
                        <div className="w-8 h-8 rounded-xl bg-amber-500/10 border border-amber-500/20 flex items-center justify-center text-amber-400"><Send className="w-4 h-4 ml-0.5"/></div>
                        <span className="text-[9px] font-bold text-amber-400 bg-amber-500/10 px-2 py-0.5 rounded-full flex items-center gap-1"><TrendingUp className="w-2.5 h-2.5"/> Live</span>
                     </div>
                     <p className="text-[10px] font-semibold tracking-widest text-zinc-400 uppercase mb-0.5 pl-2">Messages Sent</p>
                     <h3 className="text-2xl font-bold text-white tracking-tight pl-2 mt-0.5">{totalSent}</h3>
                  </div>
                </div>
                            ...
                            {colLeads.map((lead) => {
                              ...
                              return (
                                <div key={lead.id} draggable onDragStart={(e) => handleDragStart(e, lead.id)} onClick={() => setSelectedLead(lead)} 
                                  className={`group/card shrink-0 relative bg-[#374151]/60 backdrop-blur-md rounded-xl p-3.5 cursor-pointer transition-all hover:-translate-y-0.5 hover:shadow-[0_8px_20px_rgba(0,0,0,0.4)] hover:bg-[#4B5563]/60 overflow-hidden ${slaClasses}`}>
                                  ...
                                  <div className="absolute top-2.5 right-2.5 flex items-center gap-1 opacity-0 group-hover/card:opacity-100 transition-opacity z-20">
                                     <button onClick={(e) => toggleStar(e, lead.id)} className="p-1 bg-[#111827]/60 hover:bg-[#1F2937] rounded transition-colors text-zinc-400 hover:text-amber-400 border border-white/5" title="Star Lead"><Star className={`w-3 h-3 ${starredLeads.has(lead.id) ? 'fill-amber-400 text-amber-400' : ''}`} /></button>
                                     <button onClick={(e) => handleDeleteLead(e, lead.id, lead.full_name || lead.phone_number)} className="p-1 bg-[#111827]/60 hover:bg-red-500/20 rounded transition-colors text-zinc-400 hover:text-red-400 border border-white/5" title="Delete Conversation"><Trash2 className="w-3 h-3" /></button>
                                  </div>
                                  ...
                                  <div className="flex flex-col items-start mb-2.5 pl-1 pr-10 relative z-10">
                                    ...
                                    <span className={`text-[9px] ${activeTheme.text} font-bold tracking-widest mt-0.5 drop-shadow-md`}>+{lead.phone_number}</span>
                                  </div>
                                  
                                  <div className="bg-[#111827]/40 rounded-md p-2.5 ml-1 border border-white/5 group-hover/card:border-white/10 transition-colors shadow-inner">
               ...
               <div className="max-w-5xl mx-auto space-y-6">
                  {userRole === 'ADMIN' && (
                     <div className="bg-[#1F2937]/60 backdrop-blur-xl border border-white/10 rounded-2xl p-5 shadow-2xl">
                        <h3 className="text-xs font-bold text-white flex items-center gap-1.5 mb-4"><Zap className={`w-3.5 h-3.5 ${activeTheme.text}`} /> Create New Command</h3>
                        <form onSubmit={handleAddTemplate} className="flex flex-col md:flex-row gap-3">
                           <div className="w-full md:w-1/3">
                              ...
                              <input type="text" value={newShortcut} onChange={(e) => setNewShortcut(e.target.value)} placeholder="shortcut_name" className={`w-full bg-[#111827]/80 border border-white/10 rounded-lg pl-7 pr-3 py-2.5 text-xs text-white ${activeTheme.focusBorder} outline-none transition-colors`} />
                              </div>
                           </div>
                           <div className="flex-1">
                              <input type="text" value={newTemplateContent} onChange={(e) => setNewTemplateContent(e.target.value)} placeholder="Type the full message content here..." className={`w-full bg-[#111827]/80 border border-white/10 rounded-lg px-3 py-2.5 text-xs text-white ${activeTheme.focusBorder} outline-none transition-colors`} />
                           </div>
                           <button type="submit" disabled={!newShortcut.trim() || !newTemplateContent.trim()} className={`${activeTheme.bgSubtle} ${activeTheme.text} ${activeTheme.border} border ${activeTheme.hoverBg} hover:text-white disabled:opacity-50 disabled:grayscale transition-all rounded-lg px-5 py-2.5 text-xs font-bold shadow-sm`}>Save</button>
                        </form>
                     </div>
                  )}
                           ...
                           {editingTemplateId === reply.id ? (
                              <div className="flex flex-col gap-2 h-full animate-[fade-in_0.2s_ease-out]">
                                <div className="relative">
                                   ...
                                   <input type="text" value={editShortcut} onChange={(e) => setEditShortcut(e.target.value)} className={`w-full bg-[#111827]/80 border border-white/10 rounded-md pl-6 pr-2 py-1.5 text-[10px] text-white ${activeTheme.focusBorder} outline-none transition-colors`} />
                                </div>
                              ...
                              <>
                                 <div className="absolute top-2.5 right-2.5 flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                                    {userRole === 'ADMIN' && ( <button onClick={() => startEditingTemplate(reply)} className="p-1.5 bg-[#111827]/60 hover:bg-blue-500/20 rounded transition-colors text-zinc-400 hover:text-blue-400 border border-white/5" title="Edit Template"><Edit2 className="w-3 h-3" /></button> )}
                                    ...
                                    {userRole === 'ADMIN' && ( <button onClick={() => handleDeleteTemplate(reply.id)} className="p-1.5 bg-[#111827]/60 hover:bg-red-500/20 rounded transition-colors text-zinc-400 hover:text-red-400 border border-white/5" title="Delete Template"><Trash2 className="w-3 h-3" /></button> )}
                                 </div>
                                 <h4 className={`${activeTheme.text} font-bold text-xs mb-2 flex items-center gap-1`}>/{reply.shortcut}</h4>
                                 <div className="bg-[#111827]/50 rounded-lg p-3 border border-white/5 shadow-inner min-h-[60px]"><p className="text-[10px] text-zinc-300 leading-relaxed">{reply.content}</p></div>
                              </>
                           )}
                        </div>
                      ))
            ...
            <div className="flex-1 overflow-y-auto custom-scrollbar p-6">
               <div className="max-w-6xl mx-auto grid grid-cols-1 xl:grid-cols-3 gap-6">
                  <div className="xl:col-span-1 space-y-6">
                    <div className="bg-[#1F2937]/60 backdrop-blur-xl border border-white/10 rounded-2xl p-5 shadow-2xl">
                       <h3 className="text-xs font-bold text-white flex items-center gap-1.5 mb-5"><Megaphone className={`w-3.5 h-3.5 ${activeTheme.text}`} /> New Broadcast</h3>
        ...
        {activeView === 'analytics' && userRole === 'ADMIN' && (
          <div className="flex flex-col h-full animate-[fade-in_0.3s_ease-out]">
            <div className="h-20 flex items-center justify-between px-6 border-b border-white/10 bg-[#111827]/80 backdrop-blur-xl shrink-0">
              ...
              <div className="flex items-center gap-3">
                 <div className={`bg-[#1F2937] border border-white/10 rounded-lg px-3 py-1.5 flex items-center gap-1.5 text-[10px] font-semibold text-zinc-200 shadow-inner`}><Calendar className={`w-3 h-3 ${activeTheme.text}`} /> Last 30 Days</div>
              </div>
            </div>
            <div className="flex-1 overflow-y-auto custom-scrollbar p-6">
               ...
               <div className="max-w-6xl mx-auto grid grid-cols-1 xl:grid-cols-2 gap-6">
                  ...
                  <div className="space-y-6">
                    ...
                    <div className={`bg-[#1F2937]/60 backdrop-blur-xl border ${activeTheme.border} rounded-2xl p-6 shadow-2xl relative overflow-hidden`}>
                       ...
                       <div className="space-y-5 relative z-10">
                          <div>
                             <label className="text-[9px] font-bold text-zinc-400 uppercase tracking-widest pl-1">Company / Workspace Name</label>
                             <input type="text" value={settings.workspaceName} onChange={(e) => updateBranding(e.target.value, settings.accentColor)} placeholder="e.g. Acme Corp" className={`w-full mt-1 bg-[#111827]/80 border border-white/10 rounded-lg px-3 py-2 text-xs text-white ${activeTheme.focusBorder} outline-none transition-colors`} />
                          </div>
                          
                          <div>
                             <label className="text-[9px] font-bold text-zinc-400 uppercase tracking-widest pl-1 mb-2 block">Primary Accent Color</label>
                             <div className="flex items-center gap-3">
                                {Object.keys(BRAND_COLORS).map((colorKey) => (
                                   <button 
                                      key={colorKey} 
                                      onClick={() => updateBranding(settings.workspaceName, colorKey)}
                                      className={`w-6 h-6 rounded-full transition-all duration-300 border-2 ${settings.accentColor === colorKey ? 'border-white scale-110 shadow-[0_0_10px_rgba(255,255,255,0.3)]' : 'border-transparent hover:scale-110 opacity-70 hover:opacity-100'} ${BRAND_COLORS[colorKey as keyof typeof BRAND_COLORS].bg}`}
                       ...
                       <div className="space-y-5">
                          <div className="bg-[#111827]/60 rounded-xl p-5 border border-white/5">
                             <div className="flex items-center justify-between mb-3">
                                ...
                                <span className={`px-2.5 py-0.5 ${activeTheme.bgSubtle} ${activeTheme.text} text-[9px] font-bold uppercase rounded border ${activeTheme.border}`}>Configured</span>
                             </div>
                             <div className="space-y-3">
                                ...
                                <div><label className="text-[9px] font-bold text-zinc-400 uppercase tracking-widest pl-1">Phone Number ID</label><input type="text" value={settings.metaPhoneId} onChange={(e) => setSettings({...settings, metaPhoneId: e.target.value})} placeholder="e.g. 103948273948" className={`w-full mt-1 bg-[#1F2937]/80 border border-white/10 rounded-lg px-3 py-2 text-xs text-white ${activeTheme.focusBorder} outline-none transition-colors`} /></div>
                             </div>
                          </div>
                          ...
                          <div className="bg-[#111827]/60 rounded-xl p-5 border border-white/5">
                             <div className="flex items-center justify-between mb-3">
                                <h4 className="text-white font-bold text-xs flex items-center gap-1.5"><Link className="w-3.5 h-3.5 text-amber-400"/> Zapier / Make Webhooks</h4>
                                <span className={`px-2.5 py-0.5 text-[9px] font-bold uppercase rounded border ${settings.outboundWebhookUrl ? `${activeTheme.bgSubtle} ${activeTheme.text} ${activeTheme.border}` : 'bg-zinc-500/20 text-zinc-400 border-zinc-500/30'}`}>{settings.outboundWebhookUrl ? 'Active' : 'Inactive'}</span>
                             </div>
                             <div>
                                <label className="text-[9px] font-bold text-zinc-400 uppercase tracking-widest pl-1">Outbound Hook URL (Fires on RESOLVED)</label>
                                <input type="text" value={settings.outboundWebhookUrl} onChange={(e) => setSettings({...settings, outboundWebhookUrl: e.target.value})} placeholder="https://hooks.zapier.com/hooks/catch/..." className="w-full mt-1 bg-[#1F2937]/80 border border-white/10 rounded-lg px-3 py-2 text-xs text-white focus:border-amber-400 outline-none transition-colors" />
                             </div>
                          </div>
                       </div>
                    </div>
                  ...
                  <div className="space-y-6">
                     <div className={`bg-[#1F2937]/60 backdrop-blur-xl border ${activeTheme.border} rounded-2xl p-6 shadow-[0_0_30px_rgba(16,185,129,0.05)] relative overflow-hidden`}>
                        ...
                        <div className={`relative border-2 border-dashed ${activeTheme.border} bg-[#111827]/60 rounded-xl p-8 flex flex-col items-center justify-center text-center transition-all ${activeTheme.hoverBg} cursor-pointer`}>
            ...
            <div className="flex-1 flex flex-col border-r border-white/10 overflow-hidden relative">
                ...
                <div className="flex-1 overflow-y-auto p-5 flex flex-col gap-5 custom-scrollbar bg-[#0B1120]/40">
                    {chatMessages.length === 0 ? <div className="flex-1 flex items-center justify-center text-zinc-500 text-[10px] font-sans tracking-widest uppercase">No customer conversation history</div> :
                        ...
                        <div key={msg.id || i} className={`group/msg relative flex flex-col max-w-[85%] animate-[fade-in_0.3s_ease-out] ${msg.is_outbound ? 'self-end items-end' : 'self-start items-start'}`}>
                            ...
                            <div className={`p-3.5 rounded-2xl text-xs leading-relaxed shadow-lg backdrop-blur-md ${msg.is_outbound ? `bg-gradient-to-br ${activeTheme.gradient} text-white rounded-br-sm shadow-[0_5px_20px_rgba(0,0,0,0.2)] border border-white/20` : 'bg-[#1F2937] border border-white/10 text-zinc-100 rounded-bl-sm shadow-[0_5px_20px_rgba(0,0,0,0.2)]'}`}>
                               {renderMessageContent(msg.content)}
                            </div>
                            <span className={`text-[9px] font-bold tracking-widest uppercase mt-1.5 px-1 flex items-center gap-1 ${msg.is_outbound ? 'justify-end text-white/50' : 'justify-start text-zinc-500'}`}>
                                {new Date(msg.created_at).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}
                                {msg.is_outbound && <CheckCheck className={`w-3 h-3 ${msg.status === 'read' ? 'text-blue-400' : 'text-white/30'}`} />}
                            </span>
                        </div>
                    ))}
                ...
                <div className="p-5 border-t border-white/10 bg-[#111827]/90 backdrop-blur-xl shrink-0">
                          ...
                          {filteredReplies.length > 0 ? (
                            filteredReplies.map((reply) => (
                              <button key={reply.id} onClick={() => insertQuickReply(reply.content)} className="flex flex-col items-start p-2.5 rounded-lg hover:bg-white/5 transition-colors border border-transparent hover:border-white/5 text-left">
                                <span className={`text-[11px] font-bold ${activeTheme.text} mb-0.5`}>/{reply.shortcut}</span><span className="text-[10px] text-zinc-400 line-clamp-2">{reply.content}</span>
                              </button>
                            ))
                    ...
                    <form onSubmit={handleSendMessage} className="relative flex items-center group/form">
                        ...
                        <button type="submit" disabled={!newMessage.trim()} className={`absolute right-1.5 p-2 rounded-full transition-all disabled:opacity-50 disabled:grayscale ${isInternal ? 'bg-amber-500 text-black' : `bg-gradient-to-r ${activeTheme.gradient} text-white`}`}><Send className="w-3.5 h-3.5 ml-0.5"/></button>
                    </form>
                </div>
            </div>
            ...
            <div className="w-[300px] bg-[#111827]/90 backdrop-blur-xl border-l border-white/10 flex flex-col h-full shadow-inner shrink-0">
                <div className="flex-1 overflow-y-auto custom-scrollbar">
                             ...
                             {AVAILABLE_TAGS.map(tag => {
                                ...
                                return (
                                   <button 
                                      ...
                                      className={`text-[9px] font-bold uppercase tracking-widest px-2 py-1 rounded border transition-all ${isAssigned ? tag.color : 'text-zinc-500 border-white/10 hover:border-white/30 hover:text-zinc-300 bg-[#111827]/50'}`}
                                   >
                                      {tag.label}
                                   </button>
                                );
                    ...
                    <div className="p-6 pb-5 border-b border-white/10">
                        ...
                        <div className="space-y-3">
                            <div className="space-y-1"><label className={`text-[8px] font-bold ${activeTheme.text} uppercase tracking-widest ml-1`}>Assigned Name</label><input value={editProfile.full_name} onChange={(e) => setEditProfile({...editProfile, full_name: e.target.value})} onBlur={handleUpdateProfile} disabled={userRole !== 'ADMIN'} className={`w-full bg-[#1F2937] border border-white/10 rounded-lg px-3 py-2 text-[11px] text-white ${activeTheme.focusBorder} focus:bg-[#374151] outline-none transition-all disabled:opacity-70`} placeholder="Enter Full Name" /></div>
                            <div className="space-y-1"><label className={`text-[8px] font-bold ${activeTheme.text} uppercase tracking-widest ml-1`}>Email Hash</label><input value={editProfile.email} onChange={(e) => setEditProfile({...editProfile, email: e.target.value})} onBlur={handleUpdateProfile} disabled={userRole !== 'ADMIN'} className={`w-full bg-[#1F2937] border border-white/10 rounded-lg px-3 py-2 text-[11px] text-white ${activeTheme.focusBorder} focus:bg-[#374151] outline-none transition-all disabled:opacity-70`} placeholder="email@client.com" /></div>
                            <div className="space-y-1"><label className={`text-[8px] font-bold ${activeTheme.text} uppercase tracking-widest ml-1`}>Deep Notes</label><textarea rows={2} value={editProfile.profile_notes} onChange={(e) => setEditProfile({...editProfile, profile_notes: e.target.value})} onBlur={handleUpdateProfile} className={`w-full bg-[#1F2937] border border-white/10 rounded-lg px-3 py-2 text-[11px] text-white ${activeTheme.focusBorder} focus:bg-[#374151] outline-none resize-none transition-all custom-scrollbar`} placeholder="Private notes..." /></div>
                        </div>
                    </div>
                    ...
                    <div className="p-6 py-5 border-b border-white/10 bg-gradient-to-b from-[#111827] to-[#1F2937]/20">
                        ...
                        {loadingShopify ? ( <div className="p-4 bg-[#1F2937]/50 border border-white/10 rounded-lg flex items-center justify-center gap-2 text-[10px] text-zinc-400 animate-pulse shadow-inner"><Loader2 className={`w-3.5 h-3.5 animate-spin ${activeTheme.text}`} /> Fetching Shopify History...</div>
                        ) : shopifyData?.found ? (
                            <div className="space-y-3 animate-[fade-in_0.4s_ease-out]">
                                <div className={`${activeTheme.bgSubtle} border ${activeTheme.border} rounded-lg p-4 shadow-sm backdrop-blur-sm`}><p className={`text-[8px] ${activeTheme.text} opacity-80 uppercase tracking-widest font-bold mb-1`}>Lifetime Value</p><p className={`text-xl font-bold ${activeTheme.text} drop-shadow-md`}>{shopifyData.totalSpent}</p></div>
                                <div className="space-y-2"><p className="text-[8px] text-zinc-400 uppercase tracking-widest font-bold mb-2 mt-1 pl-1">Recent Orders</p>
                                    {shopifyData.recentOrders.map((order: any, idx: number) => (
                                        <div key={idx} className="bg-[#1F2937] border border-white/10 rounded-lg p-3 flex justify-between items-center hover:bg-[#374151] transition-colors shadow-sm">
                                            <div><p className="text-[11px] font-semibold text-white mb-0.5">{order.orderName}</p><p className="text-[8px] text-zinc-400 uppercase tracking-widest">{order.date} • <span className={`${activeTheme.text} opacity-80`}>{order.fulfillmentStatus || 'UNFULFILLED'}</span></p></div>
                                            <span className={`text-xs font-bold ${activeTheme.text}`}>{order.total}</span>
                                        </div>
                                    ))}
                                </div>
                            </div>
                ...
                {userRole === 'ADMIN' ? (
                   ...
                   <div className="p-5 shrink-0 border-t border-white/10 bg-[#111827]/90 backdrop-blur-md text-center">
                       <span className="text-[9px] font-bold text-zinc-500 uppercase tracking-widest flex justify-center items-center gap-1.5"><ShieldAlert className="w-3.5 h-3.5" /> Agent Mode Active</span>
                   </div>
                )}
            </div>
          </>
