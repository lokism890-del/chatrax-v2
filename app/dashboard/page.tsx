...
type ThemeColor = {
  text: string; bg: string; bgSubtle: string; border: string; borderActive: string;
  focusBorder: string; hoverBg: string; gradient: string; shadow: string;
};
...
export default function Dashboard() {
  ...
  const handleUpgrade = (planType: string) => {
    alert(`Redirecting to Lemon Squeezy Checkout for the ${planType} plan...`);
    setSubscription({ ...subscription, status: 'active', plan: planType, messageLimit: planType === 'Enterprise' ? 10000 : 2500 });
    logAudit('SUBSCRIPTION_UPDATE', `Admin upgraded to the ${planType} plan via Lemon Squeezy.`);
  };
  ...
  useEffect(() => {
    ...
    if (savedBranding) { try { const parsed = JSON.parse(savedBranding); setSettings(prev => ({ ...prev, workspaceName: parsed.name || prev.workspaceName, accentColor: parsed.color || prev.accentColor })); } catch (err) {} }
  ...
  useEffect(() => {
    ...
    const channel = supabase.channel('realtime-customers').on('postgres_changes', { event: '*', schema: 'public', table: 'customers' }, (payload) => {
        if (payload.eventType === 'INSERT') setLeads(prev => prev.find(l => l.id === payload.new.id) ? prev : [payload.new, ...prev]);
  ...
  const handleDragStart = (e: React.DragEvent, id: string) => { setDraggedLead(id); e.dataTransfer.setData('text/plain', id); e.dataTransfer.effectAllowed = 'move'; };
  ...
  const handleDrop = async (e: React.DragEvent, newStatus: string) => {
    ...
    try {
      ...
      if (!error) { logAudit('MOVE_LEAD', `Moved lead to ${newStatus}`); if (newStatus === 'RESOLVED' && settings.outboundWebhookUrl) { const lead = leads.find(l => l.id === currentDraggedId); if (lead) fireOutboundWebhook(lead); } } else { fetchLeads(); }
  ...
  const handleCSVUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = async (event) => {
      ...
      } catch (err) {} setIsUploadingCSV(false); e.target.value = ''; 
  ...
  useEffect(() => {
    ...
    const fetchShopifyData = async () => { setLoadingShopify(true); try { const response = await fetch(`/api/shopify/customer?phone=${encodeURIComponent(selectedLead.phone_number)}`); if (response.ok) { const data = await response.json(); setShopifyData(data); } } catch (err) {} setLoadingShopify(false); };
  ...
  const handleDeleteTemplate = async (id: string) => { if (!window.confirm("Delete template?")) return; try { await supabase.from('quick_replies').delete().eq('id', id); setQuickReplies(quickReplies.filter(q => q.id !== id)); } catch (err) {} };
  ...
  const handleDeleteMessage = async (msgId: string) => { if (!window.confirm("Delete this message?")) return; try { await supabase.from('messages').delete().eq('id', msgId); setMessages(prev => prev.filter(m => m.id !== msgId)); } catch (err) {} };
  const handleDeleteLead = async (e: React.MouseEvent, id: string, name: string) => { e.stopPropagation(); if (userRole !== 'ADMIN') return; if (!window.confirm('Delete lead?')) return; try { await supabase.from('customers').delete().eq('id', id); logAudit('DELETE_LEAD', `Deleted lead record for: ${name}`); setLeads(prev => prev.filter(l => l.id !== id)); if (selectedLead?.id === id) setSelectedLead(null); } catch (err) {} };
  ...
  const handleExportPDF = () => {
    ...
    doc.setFontSize(20); doc.text(`${settings.workspaceName} Intelligence Report`, 20, 20); doc.setFontSize(10); doc.text(`Subject: ${name}`, 20, 30); doc.text(`Generated: ${new Date().toLocaleString()}`, 20, 35); doc.line(20, 40, 190, 40);
  ...
  const handleLaunchCampaign = async (e: React.FormEvent) => {
    e.preventDefault(); if (!campaignName || !campaignTemplateId) return; if (!window.confirm(`Launch broadcast?`)) return;
  ...
  const searchedLeads = leads.filter(l => !globalSearch || ((l.full_name && l.full_name.toLowerCase().includes(globalSearch.toLowerCase())) || (l.phone_number && l.phone_number.includes(globalSearch.toLowerCase()))));
  ...
  // ─── RENDER HELPER FUNCTIONS (Prevents unclosed tags due to file length) ───

  const renderSidebar = () => (
    <div className="w-60 border-r border-white/5 bg-[#0F172A] flex flex-col z-40 shrink-0 h-full">
      <div className="h-20 flex items-center px-6 border-b border-white/5 shrink-0">
        <h1 className="text-lg font-extrabold tracking-tight text-white flex items-center gap-2">
          <span className={`w-7 h-7 rounded-lg bg-gradient-to-br ${activeTheme.gradient} flex items-center justify-center`}><MessageCircle className="w-4 h-4 text-white" /></span>
  ...
  const renderDashboardKanban = () => (
    <div className="flex flex-col h-full bg-[#0A101C]">
      <div className="h-20 flex items-center justify-between px-6 border-b border-white/5 shrink-0 bg-[#0F172A]/50">
        <div>
          <h2 className="text-lg font-bold text-white tracking-tight">{activeView === 'dashboard' ? 'Command Center' : 'Conversations'}</h2>
          <p className="text-[10px] text-zinc-400 font-medium mt-0.5">{activeView === 'dashboard' ? 'Live overview of your Store & CRM activity' : 'Manage and route your active customer chats'}</p>
        </div>
        <div className="flex items-center gap-3">
           {activeView === 'dashboard' ? (
              <div className={`flex items-center gap-2 bg-[#1E293B] border border-white/5 rounded-lg px-3 py-1.5 text-[10px] font-bold tracking-wider ${activeTheme.text}`}><div className={`w-1.5 h-1.5 rounded-full ${activeTheme.bg} animate-led`}></div>LIVE SYNC • {currentTime}</div>
           ) : (
              <div className="relative w-64"><Search className="w-3.5 h-3.5 text-zinc-500 absolute left-3 top-1/2 -translate-y-1/2" /><input type="text" placeholder="Search database..." value={globalSearch} onChange={(e) => setGlobalSearch(e.target.value)} className={`w-full bg-[#1E293B] border border-transparent rounded-lg pl-9 pr-3 py-1.5 text-xs text-white ${activeTheme.focusBorder} outline-none transition-colors placeholder-zinc-500`} /></div>
           )}
        </div>
      </div>
          ...
          {activeView === 'dashboard' && userRole === 'ADMIN' && (
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-6 shrink-0">
                <div className="bg-[#1E293B]/50 border border-white/5 rounded-xl p-4 relative overflow-hidden">
                   <div className={`absolute top-0 left-0 w-1 h-full ${activeTheme.bg}`} /><div className="flex justify-between items-start mb-2 pl-2"><div className={`w-7 h-7 rounded-lg ${activeTheme.bgSubtle} flex items-center justify-center ${activeTheme.text}`}><ShoppingBag className="w-3.5 h-3.5"/></div><span className={`text-[9px] font-bold ${activeTheme.text} ${activeTheme.bgSubtle} px-2 py-0.5 rounded flex items-center gap-1`}>{newOrdersPct}%</span></div>
                   <p className="text-[10px] font-medium text-zinc-500 uppercase tracking-widest mb-0.5 pl-2">New Orders</p><h3 className="text-xl font-bold text-white pl-2">{newOrdersCount}</h3>
                </div>
                <div className="bg-[#1E293B]/50 border border-white/5 rounded-xl p-4 relative overflow-hidden">
                   <div className="absolute top-0 left-0 w-1 h-full bg-sky-500" /><div className="flex justify-between items-start mb-2 pl-2"><div className="w-7 h-7 rounded-lg bg-sky-500/10 flex items-center justify-center text-sky-400"><Activity className="w-3.5 h-3.5"/></div><span className="text-[9px] font-bold text-sky-400 bg-sky-500/10 px-2 py-0.5 rounded flex items-center gap-1">{activePct}%</span></div>
                   <p className="text-[10px] font-medium text-zinc-500 uppercase tracking-widest mb-0.5 pl-2">Active Contacts</p><h3 className="text-xl font-bold text-white pl-2">{activeCount}</h3>
                </div>
                <div className="bg-[#1E293B]/50 border border-white/5 rounded-xl p-4 relative overflow-hidden">
                   <div className="absolute top-0 left-0 w-1 h-full bg-lime-500" /><div className="flex justify-between items-start mb-2 pl-2"><div className="w-7 h-7 rounded-lg bg-lime-500/10 flex items-center justify-center text-lime-400"><ShieldCheck className="w-3.5 h-3.5"/></div><span className="text-[9px] font-bold text-lime-400 bg-lime-500/10 px-2 py-0.5 rounded flex items-center gap-1">{resolvedPct}%</span></div>
                   <p className="text-[10px] font-medium text-zinc-500 uppercase tracking-widest mb-0.5 pl-2">Resolved Leads</p><h3 className="text-xl font-bold text-white pl-2">{resolvedCount}</h3>
                </div>
                <div className="bg-[#1E293B]/50 border border-white/5 rounded-xl p-4 relative overflow-hidden">
                   <div className="absolute top-0 left-0 w-1 h-full bg-amber-500" /><div className="flex justify-between items-start mb-2 pl-2"><div className="w-7 h-7 rounded-lg bg-amber-500/10 flex items-center justify-center text-amber-400"><Send className="w-3.5 h-3.5 ml-0.5"/></div><span className="text-[9px] font-bold text-amber-400 bg-amber-500/10 px-2 py-0.5 rounded flex items-center gap-1">Live</span></div>
                   <p className="text-[10px] font-medium text-zinc-500 uppercase tracking-widest mb-0.5 pl-2">Messages Sent</p><h3 className="text-xl font-bold text-white pl-2">{totalSent}</h3>
                </div>
              </div>
          )}
              ...
              return (
                <div key={status} onDragOver={handleDragOver} onDrop={(e) => handleDrop(e, status)} className="flex flex-col bg-[#0F172A] rounded-xl border border-white/5 min-h-[300px]">
                  <div className="flex items-center justify-between p-3 border-b border-white/5 shrink-0 bg-[#111827] rounded-t-xl">
                    <div className="flex items-center gap-2"><config.icon className="w-3.5 h-3.5" style={{ color: config.hex }} /><h2 className="text-[10px] font-bold tracking-widest text-zinc-400 uppercase">{status.replace('_', ' ')}</h2></div>
                       ...
                       {colLeads.map((lead) => {
                         ...
                         return (
                           <div key={lead.id} draggable onDragStart={(e) => handleDragStart(e, lead.id)} onDragEnd={handleDragEnd} onClick={() => setSelectedLead(lead)} 
                             className={`p-3.5 bg-[#1E293B] rounded-lg border cursor-grab active:cursor-grabbing hover:border-zinc-500 transition-colors ${draggedLead === lead.id ? 'opacity-40 border-dashed border-zinc-500' : 'border-white/5'} ${isSlaBreached ? 'border-red-500/30 bg-red-500/5' : ''}`}>
                             {lead.tags && lead.tags.length > 0 && (
                                <div className="flex flex-wrap gap-1 mb-2">
                                   {lead.tags.map((tagId: string) => {
                                      const tagMeta = AVAILABLE_TAGS.find(t => t.id === tagId);
                                      return tagMeta ? ( <span key={tagId} className={`text-[8px] font-bold uppercase tracking-widest px-1.5 py-[2px] rounded border ${tagMeta.color}`}>{tagMeta.label}</span> ) : null;
                                   })}
                                </div>
                             )}
                             <div className="flex justify-between items-start mb-0.5"><span className="font-sans text-sm font-semibold text-white">{lead.full_name || 'Store Customer'}</span>{isSlaBreached && <AlertTriangle className="w-3.5 h-3.5 text-red-400" />}</div>
  ...
  const renderBilling = () => (
    <div className="flex flex-col h-full bg-[#0A101C]">
      <div className="h-20 flex items-center justify-between px-6 border-b border-white/5 shrink-0 bg-[#0F172A]/50">
        <div><h2 className="text-lg font-bold text-white">Billing & Usage</h2><p className="text-[10px] text-zinc-400 font-medium mt-0.5">Manage your workspace subscription and limits</p></div>
      </div>
         ...
         <div className="max-w-4xl mx-auto space-y-6">
            <div className="bg-[#0F172A] border border-white/5 rounded-2xl p-6 relative overflow-hidden">
               ...
               <div className="relative z-10 grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div className="flex flex-col justify-center p-5 bg-[#111827] rounded-xl border border-white/5">
                     ...
                     <p className="text-xl font-bold text-white flex items-center gap-2">{subscription.plan} <span className={`text-[10px] px-2.5 py-1 rounded-full ${subscription.status === 'active' ? 'bg-emerald-500/20 text-emerald-400' : 'bg-amber-500/20 text-amber-400'}`}>{subscription.status === 'active' ? 'Active' : `${subscription.daysLeft} Days Left`}</span></p>
                     {subscription.status !== 'active' && ( <button onClick={() => handleUpgrade('Pro')} className={`mt-4 w-full py-2 bg-gradient-to-r ${activeTheme.gradient} text-white text-[10px] font-bold uppercase tracking-widest rounded-lg transition-transform hover:scale-[1.02]`}>Upgrade Now</button> )}
                  </div>
                  <div className="p-5 bg-[#111827] rounded-xl border border-white/5 flex flex-col justify-center">
                     ...
                     <div className="w-full bg-[#1E293B] rounded-full h-2.5 overflow-hidden"><div className={`${activeTheme.bg} h-full rounded-full transition-all duration-1000`} style={{ width: `${Math.min((totalMessagesUsed / subscription.messageLimit) * 100, 100)}%` }}></div></div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
               <div className="bg-[#0F172A] border border-white/5 rounded-2xl p-6 flex flex-col">
                  <h3 className="text-xl font-bold text-white mb-1">Pro Plan</h3><p className="text-xs text-zinc-500 mb-4">Perfect for growing stores.</p><p className="text-3xl font-black text-white mb-6">$99<span className="text-sm text-zinc-500 font-medium">/mo</span></p>
                  ...
                  <button onClick={() => handleUpgrade('Pro')} disabled={subscription.plan === 'Pro'} className="w-full py-2.5 rounded-lg text-xs font-bold transition-all bg-white/5 text-white hover:bg-white/10 disabled:opacity-50">{subscription.plan === 'Pro' ? 'Current Plan' : 'Select Pro'}</button>
               </div>
               <div className={`bg-[#0F172A] border ${activeTheme.border} rounded-2xl p-6 flex flex-col relative overflow-hidden`}>
                  ...
                  <h3 className="text-xl font-bold text-white mb-1">Enterprise</h3><p className="text-xs text-zinc-500 mb-4">For high-volume operations.</p><p className="text-3xl font-black text-white mb-6">$299<span className="text-sm text-zinc-500 font-medium">/mo</span></p>
                  <ul className="space-y-3 mb-8 flex-1">
                  ...
                  <button onClick={() => handleUpgrade('Enterprise')} disabled={subscription.plan === 'Enterprise'} className={`w-full py-2.5 rounded-lg text-xs font-bold transition-all bg-gradient-to-r ${activeTheme.gradient} text-white disabled:opacity-50`}>{subscription.plan === 'Enterprise' ? 'Current Plan' : 'Select Enterprise'}</button>
               </div>
            </div>
         </div>
      </div>
      ...
      <div className="flex-1 overflow-y-auto custom-scrollbar p-6">
         <div className="max-w-5xl mx-auto space-y-6">
            {userRole === 'ADMIN' && (
               <div className="bg-[#0F172A] border border-white/5 rounded-2xl p-5">
                  ...
                  <form onSubmit={handleAddTemplate} className="flex flex-col md:flex-row gap-3">
                     <div className="w-full md:w-1/3"><div className="relative"><span className="absolute left-3 top-1/2 -translate-y-1/2 text-zinc-500 font-bold text-xs">/</span><input type="text" value={newShortcut} onChange={(e) => setNewShortcut(e.target.value)} placeholder="shortcut_name" className={`w-full bg-[#1E293B] border border-transparent rounded-lg pl-7 pr-3 py-2.5 text-xs text-white ${activeTheme.focusBorder} outline-none transition-colors`} /></div></div>
                     <div className="flex-1"><input type="text" value={newTemplateContent} onChange={(e) => setNewTemplateContent(e.target.value)} placeholder="Type the full message content here..." className={`w-full bg-[#1E293B] border border-transparent rounded-lg px-3 py-2.5 text-xs text-white ${activeTheme.focusBorder} outline-none transition-colors`} /></div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              {quickReplies.length === 0 ? ( <div className="col-span-full p-6 text-center bg-[#0F172A] border border-dashed border-white/10 rounded-2xl text-zinc-500 text-xs font-medium">No templates saved yet. Create one above!</div>
              ) : (
                  ...
                  <div key={reply.id} className="group bg-[#0F172A] border border-white/5 hover:border-white/10 rounded-xl p-4 transition-colors relative min-h-[120px]">
                     {editingTemplateId === reply.id ? (
                        <div className="flex flex-col gap-2 h-full">
                          <div className="relative"><span className="absolute left-2.5 top-1/2 -translate-y-1/2 text-zinc-500 font-bold text-[10px]">/</span><input type="text" value={editShortcut} onChange={(e) => setEditShortcut(e.target.value)} className={`w-full bg-[#1E293B] border border-transparent rounded-md pl-6 pr-2 py-1.5 text-[10px] text-white ${activeTheme.focusBorder} outline-none transition-colors`} /></div>
                          ...
                          <div className="flex gap-1.5 justify-end mt-auto"><button onClick={cancelEditingTemplate} className="px-2 py-1 rounded text-[10px] font-bold text-zinc-400 hover:text-white hover:bg-white/5 transition-colors">Cancel</button><button onClick={() => handleUpdateTemplate(reply.id)} className={`px-2 py-1 rounded text-[10px] font-bold ${activeTheme.bgSubtle} ${activeTheme.text} transition-colors`}>Save</button></div>
                        </div>
                     ) : (
                        <>
                           <div className="absolute top-2.5 right-2.5 flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                              {userRole === 'ADMIN' && ( <button onClick={() => startEditingTemplate(reply)} className="p-1.5 hover:bg-[#1E293B] rounded transition-colors text-zinc-400 hover:text-white"><Edit2 className="w-3 h-3" /></button> )}
                              <button onClick={() => handleCopyTemplate(reply.id, reply.content)} className={`p-1.5 rounded transition-colors ${copiedId === reply.id ? `${activeTheme.bgSubtle} ${activeTheme.text}` : 'text-zinc-400 hover:text-white hover:bg-[#1E293B]'}`}>{copiedId === reply.id ? <Check className="w-3 h-3" /> : <Copy className="w-3 h-3" />}</button>
                              {userRole === 'ADMIN' && ( <button onClick={() => handleDeleteTemplate(reply.id)} className="p-1.5 hover:bg-red-500/10 rounded transition-colors text-zinc-400 hover:text-red-400"><Trash2 className="w-3 h-3" /></button> )}
                           </div>
  ...
  const renderSettings = () => (
    <div className="flex flex-col h-full bg-[#0A101C]">
      <div className="h-20 flex items-center justify-between px-6 border-b border-white/5 shrink-0 bg-[#0F172A]/50">
        <div><h2 className="text-lg font-bold text-white">System Settings</h2><p className="text-[10px] text-zinc-400 font-medium mt-0.5">Manage your CRM integrations and workspace data</p></div>
      </div>
      <div className="flex-1 overflow-y-auto custom-scrollbar p-6">
            ...
            <div className="space-y-6">
              <div className={`bg-[#0F172A] border ${activeTheme.border} rounded-2xl p-6 relative overflow-hidden`}>
                 ...
                 <div className="space-y-5 relative z-10">
                    <div><label className="text-[9px] font-bold text-zinc-500 uppercase tracking-widest pl-1">Company / Workspace Name</label><input type="text" value={settings.workspaceName} onChange={(e) => updateBranding(e.target.value, settings.accentColor)} placeholder="e.g. Acme Corp" className={`w-full mt-1 bg-[#1E293B] border border-transparent rounded-lg px-3 py-2 text-xs text-white ${activeTheme.focusBorder} outline-none transition-colors`} /></div>
                    <div><label className="text-[9px] font-bold text-zinc-500 uppercase tracking-widest pl-1 mb-2 block">Primary Accent Color</label>
                       <div className="flex items-center gap-3">
                          {Object.keys(BRAND_COLORS).map((colorKey) => (
                             <button key={colorKey} onClick={() => updateBranding(settings.workspaceName, colorKey)} className={`w-6 h-6 rounded-full transition-transform duration-200 border-2 ${settings.accentColor === colorKey ? 'border-white scale-110' : 'border-transparent hover:scale-110 opacity-70 hover:opacity-100'} ${BRAND_COLORS[colorKey as keyof typeof BRAND_COLORS].bg}`} />
                          ))}
                       </div>
                    </div>
                 </div>
              ...
              <div className="bg-[#0F172A] border border-white/5 rounded-2xl p-6">
                 ...
                 <div className="space-y-4">
                    <div className="bg-[#111827] rounded-xl p-4 border border-white/5">
                       <div className="flex items-center justify-between mb-3"><h4 className="text-white font-bold text-xs">WhatsApp / Meta API</h4></div>
                    ...
                    <div className="bg-[#111827] rounded-xl p-4 border border-white/5">
                       <div className="flex items-center justify-between mb-3"><h4 className="text-white font-bold text-xs">Shopify Store API</h4></div>
                    ...
                    <div className="bg-[#111827] rounded-xl p-4 border border-white/5">
                       <div className="flex items-center justify-between mb-3"><h4 className="text-white font-bold text-xs">Zapier / Make Webhooks</h4><span className={`px-2 py-0.5 text-[9px] font-bold uppercase rounded ${settings.outboundWebhookUrl ? `${activeTheme.bgSubtle} ${activeTheme.text}` : 'bg-zinc-800 text-zinc-400'}`}>{settings.outboundWebhookUrl ? 'Active' : 'Inactive'}</span></div>
                       <div><label className="text-[9px] font-bold text-zinc-500 uppercase tracking-widest pl-1">Outbound Hook URL (Fires on RESOLVED)</label><input type="text" value={settings.outboundWebhookUrl} onChange={(e) => setSettings({...settings, outboundWebhookUrl: e.target.value})} placeholder="https://hooks.zapier.com/hooks/catch/..." className="w-full mt-1 bg-[#1E293B] border border-transparent rounded-lg px-3 py-2 text-xs text-white focus:border-amber-400 outline-none transition-colors" /></div>
                    </div>
                 </div>
              </div>
            </div>
                        ...
                        <div className="space-y-3">
                           {auditLogs.map((log) => (
                              <div key={log.id} className="border-l-2 border-zinc-500/50 pl-3 py-0.5">
                                 <div className="flex justify-between items-start mb-0.5"><span className="text-zinc-300 font-bold uppercase tracking-wider">{log.action_type}</span><span className="text-zinc-600">{new Date(log.created_at).toLocaleString()}</span></div>
          ...
          <div className="flex-1 flex flex-col border-r border-white/5 overflow-hidden relative">
              <div className="flex items-center justify-between p-5 border-b border-white/5 shrink-0 bg-[#0F172A]">
                  <div className="flex items-center gap-3">
                      ...
                      <div><h3 className="text-white font-bold text-sm">{selectedLead.full_name || selectedLead.phone_number}</h3><p className={`text-[9px] ${activeTheme.text} font-bold tracking-widest uppercase mt-0.5 flex items-center gap-1.5`}><span className={`w-1.5 h-1.5 rounded-full ${activeTheme.bg} animate-pulse`}></span>Encrypted Connection</p></div>
                  </div>
              ...
              <div className="flex-1 overflow-y-auto p-5 flex flex-col gap-4 custom-scrollbar bg-[#0B1120]">
                  {chatMessages.length === 0 ? <div className="flex-1 flex items-center justify-center text-zinc-600 text-[10px] font-sans tracking-widest uppercase">No conversation history</div> :
                      ...
                      <div key={msg.id || i} className={`group/msg relative flex flex-col max-w-[85%] ${msg.is_outbound ? 'self-end items-end' : 'self-start items-start'}`}>
                          {userRole === 'ADMIN' && <button onClick={() => handleDeleteMessage(msg.id)} className={`absolute top-1/2 -translate-y-1/2 ${msg.is_outbound ? '-left-8' : '-right-8'} opacity-0 group-hover/msg:opacity-100 p-1 hover:bg-red-500/20 rounded text-zinc-600 hover:text-red-400`}><Trash2 className="w-3 h-3" /></button>}
                          <div className={`p-3.5 rounded-xl text-sm leading-relaxed ${msg.is_outbound ? `bg-gradient-to-br ${activeTheme.gradient} text-white rounded-tr-sm` : 'bg-[#1E293B] text-zinc-200 rounded-tl-sm'}`}>{renderMessageContent(msg.content)}</div>
                          <span className={`text-[9px] font-medium mt-1 px-1 flex items-center gap-1 ${msg.is_outbound ? 'justify-end text-zinc-500' : 'justify-start text-zinc-500'}`}>{new Date(msg.created_at).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}{msg.is_outbound && <CheckCheck className={`w-3 h-3 ${msg.status === 'read' ? 'text-blue-400' : 'text-zinc-600'}`} />}</span>
                      </div>
                  ))}
                  ...
                  {showCommandMenu && (
                    <div className="absolute bottom-[80px] left-4 mb-2 w-72 max-h-48 overflow-y-auto custom-scrollbar bg-[#1E293B] border border-white/10 rounded-lg shadow-xl z-50">
                      ...
                      <div className="p-1 flex flex-col">
                        {filteredReplies.length > 0 ? filteredReplies.map((reply) => (<button key={reply.id} onClick={() => insertQuickReply(reply.content)} className="flex flex-col items-start p-2 rounded hover:bg-white/5 text-left"><span className={`text-[10px] font-bold ${activeTheme.text}`}>/{reply.shortcut}</span><span className="text-[10px] text-zinc-400 truncate w-full">{reply.content}</span></button>)) : ( <div className="p-3 text-center text-[10px] text-zinc-500">No matches.</div> )}
                      </div>
                    </div>
                  )}
          ...
          <div className="w-[300px] bg-[#0F172A] border-l border-white/5 flex flex-col h-full shrink-0">
                     ...
                     <div className="p-5 border-b border-white/5 bg-[#111827]/50">
                        ...
                        <div className="flex flex-wrap gap-1.5">
                           {AVAILABLE_TAGS.map(tag => {
                              ...
                              return <button key={tag.id} onClick={() => handleToggleTag(tag.id)} className={`text-[9px] font-bold uppercase tracking-widest px-2 py-1 rounded transition-colors ${isAssigned ? tag.color : 'text-zinc-500 bg-[#1E293B] hover:text-zinc-300'}`}>{tag.label}</button>;
                           })}
                        </div>
                     </div>
                  )}
                          ...
                          {internalMemos.length === 0 ? ( <div className="p-3 rounded-lg bg-[#111827]/50 text-center"><p className="text-[9px] text-zinc-600 uppercase tracking-widest font-bold">No internal memos</p></div>
                              ...
                              internalMemos.map((memo, i) => (
                                  <div key={memo.id} className="group relative bg-amber-500/10 rounded-lg p-2.5">
                                      {userRole === 'ADMIN' && <button onClick={() => handleDeleteMemo(memo.id)} className="absolute top-1.5 right-1.5 opacity-0 group-hover:opacity-100 p-1 hover:bg-red-500/20 rounded text-amber-500 hover:text-red-400 transition-colors"><Trash2 className="w-3 h-3" /></button>}
                                      <p className="text-[10px] text-amber-100/90 leading-relaxed mb-1 pr-4">{memo.content}</p>
                                      <span className="text-[8px] text-amber-500/50 font-bold uppercase tracking-widest block text-right">{new Date(memo.created_at).toLocaleString()}</span>
                                  </div>
                              ))
                          )}
                      </div>
              ...
              {userRole === 'ADMIN' ? (
                 <div className="p-4 shrink-0 border-t border-white/5 bg-[#0F172A]">
                     <button type="button" onClick={handleExportPDF} className="w-full flex items-center justify-center gap-1.5 py-2.5 bg-[#1E293B] hover:bg-white/10 rounded-lg text-[9px] font-bold uppercase tracking-widest text-zinc-300 transition-colors"><Download className="w-3.5 h-3.5"/> Export Intelligence</button>
                 </div>
              ) : (
                 <div className="p-4 shrink-0 border-t border-white/5 bg-[#0F172A] text-center"><span className="text-[9px] font-bold text-zinc-600 uppercase tracking-widest flex justify-center items-center gap-1.5"><ShieldAlert className="w-3.5 h-3.5" /> Agent Mode Active</span></div>
              )}
          </div>
        </>
      )}
