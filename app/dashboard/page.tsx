...
export default function CRMDashboard() {
  ...
  const handleUpgrade = (plan: string) => { 
    if(LEMON_SQUEEZY_LINKS[plan] && !LEMON_SQUEEZY_LINKS[plan].includes('YOUR_')) { 
      window.location.href = LEMON_SQUEEZY_LINKS[plan]; 
      logAudit('SUBSCRIPTION', `Upgrade to ${plan}`); 
    } else {
      alert("Lemon Squeezy URL missing in configuration."); 
    }
  };
  ...
  const updateBranding = (name: string, color: string) => { 
    setSettings((s: any) => ({ ...s, workspaceName: name, accentColor: color })); 
    localStorage.setItem('chatrax_brand', name);
    localStorage.setItem('chatrax_theme', color);
  };
  ...
  const handleCSVUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = async (ev) => { 
      try { 
        ...
        if (nL.length > 0) { await supabase.from('customers').insert(nL); logAudit('SYSTEM_MIGRATION', `Imported ${nL.length} leads.`); fetchLeads(); alert(`Imported ${nL.length} leads.`); } 
  ...
  const RenderDashboard = () => (
    <div className="space-y-6 animate-[fade-in_0.3s_ease-out] p-6 h-full overflow-y-auto hover-scroll">
        ...
        <div className={`p-4 border-b ${ui.border}`}>
          ...
          <div className="flex gap-2 overflow-x-auto hide-scrollbar pb-1">
            {['All', 'Open', 'Follow-up', 'Resolved'].map(f => (
              <button key={f} className={`whitespace-nowrap px-3 py-1.5 rounded-lg text-[10px] font-bold tracking-wider uppercase border ${f === 'All' ? `${t.bg} text-white border-transparent shadow-sm` : `bg-transparent ${ui.textMuted} ${ui.border} ${ui.hover}`}`}>{f}</button>
            ))}
          </div>
        </div>
          ...
          <>
            ...
            <div className={`flex-1 flex flex-col ${ui.bgMain} relative`}>
              <div className={`p-4 border-b ${ui.border} flex justify-between items-center ${ui.bgSidebar}`}>
                ...
                <div className="flex items-center gap-2">
                  {selectedLead.status === 'NEW_ORDER' && <button onClick={async () => { await supabase.from('customers').update({status:'ACTIVE'}).eq('id',selectedLead.id); setLeads((pr:any[])=>pr.map(l=>l.id===selectedLead.id?{...l,status:'ACTIVE'}:l)); setSelectedLead((pr:any)=>pr?{...pr,status:'ACTIVE'}:null); }} className={`px-3 py-1.5 rounded-lg border border-sky-500/30 text-sky-500 hover:bg-sky-500/10 text-xs font-bold flex items-center gap-1.5 transition-colors`}><CheckCircle2 className="w-4 h-4" /> Take Over</button>}
                  <button onClick={async () => { await supabase.from('customers').update({status:'RESOLVED'}).eq('id',selectedLead.id); setLeads((pr:any[])=>pr.map(l=>l.id===selectedLead.id?{...l,status:'RESOLVED'}:l)); setSelectedLead(null); }} className={`px-3 py-1.5 rounded-lg border border-emerald-500/30 text-emerald-500 hover:bg-emerald-500/10 text-xs font-bold flex items-center gap-1.5 transition-colors`}><CheckCircle2 className="w-4 h-4" /> Resolve</button>
              ...
              <div className={`flex-1 p-6 overflow-y-auto hover-scroll flex flex-col gap-4 ${ui.bgMain}`}>
                {messages.length === 0 ? <div className={`flex-1 flex items-center justify-center text-xs font-bold tracking-widest uppercase ${ui.textMuted}`}>Start the conversation</div> : 
                 messages.map((msg: any) => (
                  <div key={msg.id} className={`group/msg relative flex flex-col max-w-[80%] ${msg.is_outbound ? 'self-end items-end' : 'self-start items-start'}`}>
                    {userRole === 'ADMIN' && <button onClick={async () => { if(window.confirm('Delete?')){ await supabase.from('messages').delete().eq('id', msg.id); setMessages((pr: any[]) => pr.filter(m=>m.id!==msg.id)); } }} className={`absolute top-1/2 -translate-y-1/2 ${msg.is_outbound ? '-left-12' : '-right-12'} opacity-0 group-hover/msg:opacity-100 p-2 rounded-xl transition-all ${isDarkMode ? 'hover:bg-red-500/20 text-slate-400 hover:text-red-400' : 'hover:bg-red-50 text-slate-400 hover:text-red-500'}`}><Trash2 className="w-4 h-4" /></button>}
                    <div className={`p-3.5 rounded-2xl text-[13px] leading-relaxed shadow-sm ${msg.is_outbound ? (msg.is_internal ? 'bg-amber-500/20 text-amber-500 border border-amber-500/30 rounded-br-sm' : `${t.bg} text-white rounded-br-sm`) : `${ui.card} ${ui.textMain} border ${ui.border} rounded-bl-sm`}`}>
              ...
              <div className="p-5 space-y-6">
                ...
                <div>
                  ...
                  <div className="flex flex-wrap gap-2">
                    {selectedLead.tags?.map((tg: string) => <span key={tg} className={`px-2.5 py-1 rounded-md border ${ui.border} ${ui.card} text-[10px] font-bold ${ui.textMuted}`}>{tg}</span>)}
                    <button className={`px-2.5 py-1 rounded-md border border-dashed ${ui.border} text-[10px] font-bold ${ui.textMuted} hover:border-amber-500 hover:${ui.accentText} transition-colors`}><Plus className="w-3 h-3"/></button>
                  </div>
                </div>
                <div>
                  <h4 className={`text-[10px] font-bold uppercase tracking-widest ${ui.textMuted} mb-3`}>Internal Notes</h4>
                  <textarea value={selectedLead.profile_notes || ''} onChange={(e) => { setSelectedLead((pr:any)=>({...pr, profile_notes: e.target.value})); }} onBlur={async () => { await supabase.from('customers').update({profile_notes: selectedLead.profile_notes}).eq('id', selectedLead.id); }} placeholder="Add a note..." className={`w-full ${ui.input} rounded-xl px-3 py-2 text-xs outline-none resize-none hover-scroll h-24`} />
                </div>
              </div>
            </div>
          </>
  ...
  const RenderContacts = () => (
    <div className="p-6 h-full overflow-y-auto hover-scroll animate-[fade-in_0.3s_ease-out]">
  ...
  const RenderTemplates = () => (
    <PageWrapper title="Template Commands" subtitle="Manage your team's quick replies." ui={ui}>
  ...
  const RenderTeam = () => (
    <PageWrapper title="Team Management" subtitle="Monitor agent performance and routing." ui={ui}>
  ...
  const RenderCampaigns = () => (
    <PageWrapper title="Campaigns" subtitle="Send mass updates." ui={ui}>
  ...
  const RenderAnalytics = () => (
    <PageWrapper title="Analytics" subtitle="Performance metrics" rightHeader={<div className={`${ui.card} rounded-xl px-4 py-2 border ${ui.border} flex items-center gap-2 text-xs font-bold ${ui.textMuted} shadow-sm`}><Calendar className="w-4 h-4" /> 30 Days</div>} ui={ui}>
  ...
  const RenderSettings = () => (
    <PageWrapper title="Settings" subtitle="Manage your workspace." ui={ui}>
  ...
  const RenderPlaceholder = (title: string, IconComponent: any, desc: string) => (
    <div className="p-6 h-full flex flex-col animate-[fade-in_0.3s_ease-out]">
       <div className={`flex-1 flex flex-col items-center justify-center ${ui.card} rounded-3xl border ${ui.border} shadow-sm text-center`}>
         <div className={`w-24 h-24 rounded-full ${t.sub} flex items-center justify-center mb-6`}><IconComponent className={`w-10 h-10 ${t.text}`} /></div>
         <h2 className={`text-2xl font-bold ${ui.textMain} mb-3 capitalize`}>{title} Module</h2>
         <p className={`text-sm ${ui.textMuted} max-w-md leading-relaxed`}>{desc}</p>
       </div>
    </div>
  );
  ...
  return (
    <div className={`flex h-screen w-screen font-sans ${ui.bgMain} overflow-hidden selection:bg-amber-500/30 transition-colors duration-500`}>
      ...
      {/* ─── SIDEBAR ─── */}
      <div className={`w-[240px] m-4 mr-0 rounded-2xl ${ui.bgSidebar} border ${ui.border} flex flex-col z-40 shadow-xl shrink-0 transition-colors duration-300`}>        <div className={`h-20 flex items-center px-6 shrink-0 border-b ${ui.border}`}>
          <h1 className={`text-2xl font-black tracking-tight ${ui.textMain} flex items-center gap-2`}>
            <MessageSquare className={`w-6 h-6 ${t.text}`} /> Chat<span className={t.text}>Rax</span>
          </h1>
        </div>
        <div className="flex-1 py-6 px-4 space-y-1 overflow-y-auto hover-scroll">
          ...
          {NAV_MENU.slice(0, 6).map((item) => (
            <button key={item.id} onClick={() => setActiveView(item.id)} className={`w-full flex items-center justify-between px-4 py-3 rounded-xl transition-all duration-300 font-bold text-xs ${activeView === item.id ? `${t.bg} text-white shadow-md` : `${ui.textMuted} ${ui.hover}`}`}>
               <div className="flex items-center gap-3"><item.icon className={`w-4 h-4 ${activeView === item.id ? '' : 'opacity-70'}`} /> {item.label}</div>
               {item.id === 'inbox' && <span className={`text-[9px] px-2 py-0.5 rounded-full ${activeView === item.id ? 'bg-white/20' : 'bg-slate-800 text-amber-400'} font-black`}>{leads.filter((l:any)=>l.status!=='RESOLVED').length}</span>}
            </button>
          ))}
      ...
      <div className="flex-1 flex flex-col h-full overflow-hidden p-4 pb-4">
        ...
        {/* TOP HEADER */}
        <div className={`h-16 mb-4 rounded-2xl ${ui.card} border ${ui.border} flex items-center justify-between px-6 shrink-0 shadow-sm transition-colors duration-300`}>
           ...
           <div className="flex items-center gap-2">
              ...
              <div className="relative">
                <button onClick={() => setShowNotifications(!showNotifications)} className={`p-2 rounded-full ${ui.textMuted} ${ui.hover} relative transition-colors`}>
                  ...
                  {leads.filter((l:any)=>l.status==='NEW_ORDER').length > 0 && <span className="absolute top-1.5 right-2 w-2 h-2 bg-red-500 rounded-full border border-[#1F2937]"></span>}
                </button>
                {showNotifications && (
        ...
        <div className="flex-1 overflow-hidden relative">
          ...
          {['deals', 'automation', 'reports'].includes(activeView) && RenderPlaceholder(activeView, Settings, 'This module is currently being configured for your workspace.')}
        </div>
      </div>
    </div>
  );
